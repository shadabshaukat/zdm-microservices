...
def validate_saved_job_record(
    ...
    *,
    ...
    if not name:
        raise ValueError("Invalid saved-jobs API response: saved job key must be a non-empty string.")
    if not isinstance(job, Mapping):
        raise ValueError(f"Invalid saved-jobs API response: saved job '{name}' must be an object.")
    ...
    if keys != SAVED_JOB_RECORD_FIELDS:
        ...
        raise ValueError(
            f"Invalid saved-jobs API response: saved job '{name}' has invalid fields ({'; '.join(details)})."
        )
    ...
    if not job_name:
        raise ValueError(f"Invalid saved-jobs API response: saved job '{name}' is missing name.")
    if enforce_key_match and job_name != name:
        raise ValueError(
            f"Invalid saved-jobs API response: saved job key '{name}' does not match record name '{job_name}'."
        )
    if not project:
        raise ValueError(f"Invalid saved-jobs API response: saved job '{name}' is missing project.")
    if run_type not in SAVED_JOB_RUN_TYPES:
        raise ValueError(f"Invalid saved-jobs API response: saved job '{name}' has invalid run_type.")
    expected_name = canonical_saved_job_name(project, run_type)
    if job_name != expected_name:
        raise ValueError(
            f"Invalid saved-jobs API response: saved job '{name}' must be named '{expected_name}'."
        )
    if not isinstance(job.get("job_parameters"), Mapping):
        raise ValueError(
            f"Invalid saved-jobs API response: saved job '{name}' job_parameters must be an object."
        )

    ignore = job.get("ignore")
    if ignore is not None and not isinstance(ignore, list):
        raise ValueError(f"Invalid saved-jobs API response: saved job '{name}' ignore must be a list.")
    custom_args = job.get("custom_args")
    if custom_args is not None and not isinstance(custom_args, list):
        raise ValueError(f"Invalid saved-jobs API response: saved job '{name}' custom_args must be a list.")
    listphases = job.get("listphases")
    if listphases is not None and not isinstance(listphases, bool):
        raise ValueError(f"Invalid saved-jobs API response: saved job '{name}' listphases must be a boolean.")
...
def validate_saved_jobs_response(payload: Any) -> Dict[str, Dict[str, Any]]:
    if not isinstance(payload, Mapping):
        raise ValueError("Invalid saved-jobs API response: expected an object keyed by saved job name.")
    return {
...
def validate_saved_job_save_response(payload: Any, expected_name: Any) -> Dict[str, Any]:
    ...
    if not isinstance(payload, Mapping):
        raise ValueError("Invalid saved-jobs save API response: expected an object.")
    ...
    if keys != SAVED_JOB_SAVE_RESPONSE_FIELDS:
        ...
        if extra:
            details.append("unexpected " + ", ".join(extra))
        raise ValueError(f"Invalid saved-jobs save API response: invalid fields ({'; '.join(details)}).")
    if payload.get("status") != "success":
        raise ValueError("Invalid saved-jobs save API response: status must be success.")
...
def validate_saved_job_delete_response(payload: Any) -> Dict[str, Any]:
    if not isinstance(payload, Mapping):
        raise ValueError("Invalid saved-jobs delete API response: expected an object.")
    keys = set(str(key) for key in payload.keys())
    if keys != SAVED_JOB_DELETE_RESPONSE_FIELDS:
        ...
        if extra:
            details.append("unexpected " + ", ".join(extra))
        raise ValueError(f"Invalid saved-jobs delete API response: invalid fields ({'; '.join(details)}).")
    if payload.get("status") != "success":
        raise ValueError("Invalid saved-jobs delete API response: status must be success.")
    if not isinstance(payload.get("message"), str) or not payload.get("message"):
        raise ValueError("Invalid saved-jobs delete API response: message must be a non-empty string.")
    return dict(payload)
...
def validate_saved_job_copy_response(
    ...
    *,
    ...
    if not isinstance(payload, Mapping):
        raise ValueError("Invalid saved-jobs copy API response: expected an object.")
    keys = set(str(key) for key in payload.keys())
    if keys != SAVED_JOB_COPY_RESPONSE_FIELDS:
        ...
        if extra:
            details.append("unexpected " + ", ".join(extra))
        raise ValueError(f"Invalid saved-jobs copy API response: invalid fields ({'; '.join(details)}).")
    if payload.get("status") != "success":
        raise ValueError("Invalid saved-jobs copy API response: status must be success.")
    if payload.get("source_project") != expected_source_project:
        raise ValueError("Invalid saved-jobs copy API response: source_project mismatch.")
    if payload.get("target_project") != expected_target_project:
        raise ValueError("Invalid saved-jobs copy API response: target_project mismatch.")
    if str(payload.get("migration_method") or "").strip().upper() != str(expected_method or "").strip().upper():
        raise ValueError("Invalid saved-jobs copy API response: migration_method mismatch.")
    if str(payload.get("run_type") or "").strip().upper() != str(expected_run_type or "").strip().upper():
        raise ValueError("Invalid saved-jobs copy API response: run_type mismatch.")
